...
### Added
- **Per-run detail logs.** Each headless `claude -p` merge/evolve call now writes a structured log under `~/.claude/brain-runs/` capturing the model, budget, duration, exit code, raw stderr, and the (truncated) response payload — the diagnostics that previously vanished into the Claude Code session list. Logs are created atomically `0600` (via a scoped `umask`, since they contain merged brain content + stderr) with a PID-suffixed filename (so two same-action runs in the same second don't collide), and are machine-local: only the basename is recorded in the synced `meta/merge-log.json`, so no username/filesystem layout leaks across machines. `/brain-log verbose [<n>]` surfaces the detail log for a given entry.
